...
export async function StatesResponse(
    ...
    let states = statesData.data.map((state) => {
        return {
            ...state,
            recovered:
                getStateById(statesRecoverdData, state.id)?.recovered ?? 0,
            ...
            delta: {
                ...
                deaths:
                    getStateById(statesNewDeathsData, state.id)?.deaths ?? 0,
                recovered:
                    getStateById(statesNewRecoveredData, state.id)?.recovered ??
                    0,
            },
        };
    });
