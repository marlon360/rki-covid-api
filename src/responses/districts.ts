...
export async function DistrictsResponse(ags?: string): Promise<DistrictsData> {
    ...
    let districts = districtsData.data.map((district) => {
        return {
            ...
            recovered:
                getDistrictByAgs(districtsRecoveredData, district.ags)
                    ?.recovered ?? 0,
            weekIncidence:
            ...
            delta: {
                cases:
                    getDistrictByAgs(districtNewCasesData, district.ags)
                        ?.cases ?? 0,
                deaths:
                    getDistrictByAgs(districtNewDeathsData, district.ags)
                        ?.deaths ?? 0,
                recovered:
                    getDistrictByAgs(districtNewRecoveredData, district.ags)
                        ?.recovered ?? 0,
            },
        };
    });
